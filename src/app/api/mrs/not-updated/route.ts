...
import { fetchNotUpdatedMRs } from "@/lib/gitlab";
import { Cache } from "@/lib/cache";
import { logger } from "@/lib/logger";
import { thresholds } from "@/lib/config";

// Create a cache for not-updated MRs with 60 seconds TTL
...
export async function GET(request: NextRequest) {
  ...
  const threshold = customThreshold
    ...
    : thresholds.NOT_UPDATED_THRESHOLD;

  // Check for cache-busting refresh parameter
  const shouldRefresh = searchParams.get("refresh") === "true";

  // Create cache key based on page, per_page and threshold
  const cacheKey = `not-updated-mrs-page-${page}-per-${per_page}-threshold-${threshold}`;
  ...
  try {
    logger.info(
      "Fetching not-updated MRs from GitLab",
      { page, per_page, threshold },
      "API"
    );

    // Fetch MRs from GitLab API
const response = await fetchNotUpdatedMRs({
  ...
  threshold,
    ...
    logger.error("Error fetching not-updated MRs", { error }, "API");

    return NextResponse.json(
      {
        error: "Failed to fetch not-updated MRs",
